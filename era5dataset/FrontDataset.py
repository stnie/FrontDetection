...
def datanameToLabelnameFold(filename, mapTypes):
    return {key: os.path.join(str(x[0]), os.path.splitext(filename)[0][3:]+".txt") for key, x in mapTypes.items()}
def datanameToLabelname(filename, mapTypes):
    return {key: os.path.join(str(x[0]), os.path.splitext(filename)[0]+".txt") for key, x in mapTypes.items()}
def datanameToLabelname2(filename, mapTypes, removePrefix):
...
class WeatherFrontDataset(Dataset):
    ...
    def __init__(self, data_dir, label_dir = None, mapTypes = {"NA": ("", (35,70), (-40,35), (0.25,0.25), (1,1), None) }, levelRange = None, transform=None, outSize = None, printFileName = False, labelThickness = 2, label_extractor = None, asCoords = False, era_extractor = DefaultEraExtractor, has_subfolds = (False, False), removePrefix = 0):
        ...
        self.levelrange = levelRange #np.arange(105,138,4)
        #self.levelrange = np.arange(113,126,2)
        # Latrange of era to extract for each mapType (used for crop)
            ...
            for fold in os.listdir(self.data_dir):
                for filen in os.listdir(os.path.join(self.data_dir, fold)):
                    ...
                    if(self.removePrefix == 8 and not ( "_00" in filen or "_06" in filen or "_12" in filen or "_18" in filen)):
                        ...
                        if(self.has_label):
                            potLabel = datanameToLabelname2(filen, self.mapTypes, self.removePrefix)
                            labelExists = False
        ...
        #self.estimateLabelDistribution()
        
    
    def estimateLabelDistribution(self):  
        # check how labels are distributed in the dataset  
        totshap = np.zeros((2,4))
        totlen = np.zeros((2,4))
        for fidx, myfile in enumerate(self.fileList):
            print(fidx)
            idx = 0
            for mapType, val in self.mapTypes.items():
                fronts = self.getLabel(os.path.join(self.label_dir,datanameToLabelname2(myfile, self.mapTypes, self.removePrefix)[mapType]), self.mapTypes[mapType][1], self.mapTypes[mapType][2], self.mapTypes[mapType][3], mapType, 0)
                for groupidx, group in enumerate(fronts):
                    for front in group:
                        totshap[idx, groupidx] += 1
                        totlen[idx,groupidx] += np.sum(np.linalg.norm(front[:-1] - front[1:], axis = 1))
                idx+=1
    ...
    def __getitem__(self, idx):
        ...
        if(self.has_label):
            ...
            if(self.hasSubfolds[1]):
                front_name = datanameToLabelname2(filepath, self.mapTypes, self.removePrefix)
            else:
                if(self.hasSubfolds[0]):
                    front_name = datanameToLabelname2(filename, self.mapTypes, self.removePrefix)
                else:
                    front_name = datanameToLabelname2(filename, self.mapTypes, self.removePrefix)
    ...
    def getRegularGridImage(self, filename, latrange, lonrange, res, seed, tseed = 0):
        warpmask = self.getRegularGridMask(latrange, lonrange, res, seed)
        return self.era_extractor(filename, None, None, self.levelrange, tseed, warpmask)
    
    def getRegularGridLabel(self, filename, latrange, lonrange, res, types, seed):
        warpmask = self.getRegularGridMask(latrange, lonrange, res, seed)
        return self.label_extractor(filename, None, None, res, types, warpmask)

    def getRegularGridMask(self, latrange, lonrange, res, seed):
                # Set same seed for random crop in image and label extraction
        common_seed = seed
        random.seed(common_seed)

        # th and tw are the desired image size (post crop)
        th,tw = self.cropsize

        # The total amount of pixel in the desired latitude range of the lat lon image
        totLat = (latrange[1]-latrange[0]) / res[0] 
        # The resolution of latitude in kilometer
        kmPerLat = LatTokmPerLon(0)
        # How many kilometer are in the desired latitude range
        kmInLatRange = totLat * kmPerLat 

        # How many potential pixel can fit in the latitude range with the given gridStep (desired km distance between pixel)
        gridStep = 20#kmPerLat
        gridPointsinLat = kmInLatRange / gridStep
        
        # h is the maximum number of points that can be found in the given latrange (height of the latrange in km image)
        h = int(gridPointsinLat)+1

        # The lower position of the image in km space
        i = random.randint(0, h-th)
        # The corresponding upper end
        ii = i+th

        # the ends need to be transformed into lat lon space to get the km distortion at given lats
        beginLat = latrange[0]+i*gridStep/kmPerLat * res[0]

        # The latitude positions (pixel in lat lon image) of each pixel in the km image
        latmask = np.arange(beginLat, beginLat+gridStep/kmPerLat*res[0]*(th-0.1), gridStep/kmPerLat*res[0])
        
        # Get All longitude distortions within the latitude range
        lonDist = LatTokmPerLon(latmask)
        # Get the maximum distortion to estimate crop position for longitude
        kmPerLon = np.max(lonDist)

        # The total amount of pixel in the lat lon image (lon direction)
        totLon = (lonrange[1]-lonrange[0]) / res[1] 
        kmInLonRange = totLon * kmPerLon 
        gridPointsinLon = kmInLonRange / gridStep

        # w is the maximum number of points that can be found in the given lonrange  (this is decided by the lowest resolution latitude)
        w = int(gridPointsinLon)+1

        # the left position in image space
        j = random.randint(0, w-tw)

        leftLon = lonrange[0] + gridStep / kmPerLon * j * res[1]
        
        # The longitued positions (pixel in lat lon image) of each pixel in the km image
        lonmask = np.arange(leftLon, leftLon+gridStep/kmPerLon*res[1]*(tw- 0.1), gridStep/kmPerLon*res[1])

        # combine latmask and lonmask into a warping mask
        # latmask can be broadcast to cropsize
        latmask = np.broadcast_to(np.reshape(latmask, (th,1)), (th, tw))
        # lonmask needs to be distorte before being broadcast. 
        # The distortion also makes a modulo operation necessary, to bring the values in the desired -180 to 180 range
        lonmask = np.mod(np.broadcast_to(lonmask, (th, tw))*np.reshape(kmPerLat/lonDist, (th, 1))+180, 360)-180

        warpmask= np.array([latmask, lonmask])
        return warpmask
    ...
    def getBatch(self, idx, sname = False):
        lt = LabeledTimerDummy()
        
        filenames = np.array(os.listdir(self.data_dir))[idx]
        # all image names in the slice
        img_names = [os.path.join(self.data_dir, filename )for filename in filenames]
        # all corresponding front names
        front_names = [datanameToLabelname(filename, self.types) for filename in filenames]
        projection_type = 0
        for fid, filename in enumerate(front_names):
            if(filename[0] in os.listdir(self.label_dir)):
                front_names[fid] = os.path.join(self.label_dir, filename[0])
            else:
                print(front_names, filename[0])
                print(os.listdir(self.label_dir))
                print("Invalid label data pair, no label found!")
        if(sname):
            print(idx)
            ...
            print(front_names)
            print()
        batchsize = len(front_names)
        # extract data within the valid range depending on projection type (NA or NT)
        # Crop the ranges we want to extract instead of cropping the resulting image => less memory overhead
        if(self.transform):
            common_seeds = [0 for x in range(batchsize)]
            h,w = len(self.latrange[projection_type]), len(self.lonrange[projection_type])
            th,tw = self.outSize[0]
            latranges = np.zeros((batchsize, th), dtype=int)
            lonranges = np.zeros((batchsize, tw), dtype=int)
            for entry in range(batchsize):
                common_seeds[entry] = datetime.now()
                random.seed(common_seeds[entry])
                i = random.randint(0, h-th)
                j = random.randint(0, w-tw)
                latranges[entry] = self.latrange[projection_type][i:i+th]
                lonranges[entry] = self.lonrange[projection_type][j:j+tw]

            
        image = readNC(img_names, latranges, lonranges)
        
        #image = readNC(img_names, self.latrange[projection_type], self.lonrange[projection_type])
        fronts = readCSB(front_names, self.latrange[projection_type], self.lonrange[projection_type], self.types[projection_type], self.binary)
        # Perform transformation on the data (affine transformation + randm crop) => Crop enables equally sized images

        if self.transform:
            # draw a random seed so we can reset the random generator so perform the same operation on image and label data
            outSZ = self.outSize[0]
            loutSZ = self.outSize[1]
            if(self.outSize == None):
                outSZ = (len(self.latrange[projection_type]), len(self.lonrange[projection_type])) 
            
            
            sample = []
            for batchID in range(image.shape[0]):
                finalImage = np.zeros((image.shape[1], image.shape[2], *outSZ))
                for channel in range(image.shape[1]):
                    for level in range(image.shape[2]):
                        random.seed(common_seeds[batchID])
                        x = random.randint(0,1)
                        y = random.randint(0,1)
                        finalImage[channel, level,:,:] = self.transform[0](image[batchID,channel,level,:,:])
                finalFronts = np.zeros((*loutSZ, fronts.shape[3]))
                for channel in range(fronts.shape[3]):
                    random.seed(common_seeds[batchID])
                    finalFronts[:,:,channel] = self.transform[1](fronts[batchID,:,:,channel])
                sample.append([torch.from_numpy(finalImage), torch.from_numpy(finalFronts)])
            return sample
        else:
            sample = [torch.from_numpy(image), torch.from_numpy(fronts)]
        return sample
...
class WeatherFrontBatch:
    def __init__ (self, data, label_as_float = True, transpose_rate = 0.5, swap_indices = None):
        ...
        if(transpose<transpose_rate):
            self.data = torch.stack(transposed_data[0],0).permute(0,1,3,2).float()
            self.swapPairs(self.data, swap_indices)
            if(transposed_data[1][0] is None):
                self.labels = None
            else:
                if(label_as_float):
                    self.labels = torch.stack(transposed_data[1],0).permute(0,2,1,3).float()
                else:
                    self.labels = torch.stack(transposed_data[1],0).permute(0,2,1,3).long()
        else:
    ...
    def swapPairs(self, data, index_info):
        for info in index_info:
            udir_idxRange, vdir_idxRange = info
            target_range = np.concatenate((udir_idxRange,vdir_idxRange))
            source_range = np.concatenate((vdir_idxRange,udir_idxRange))
            data[:, target_range] = data[:, source_range]
...
class WeatherFrontsAsCoordinatesBatch:
    def __init__ (self, data, label_as_float = True, transpose_rate = 0.5, swap_indices = None):
        transposed_data = (list(zip(*data)))
        transpose = random.random()
        if(False and transpose<transpose_rate):
            self.data = torch.stack(transposed_data[0],0).permute(0,1,3,2).float()
            self.swapPairs(self.data, swap_indices)
            if(transposed_data[1][0] is None):
                self.labels = None
    ...
    def pin_memory(self):
        self.data = self.data.pin_memory()
        #self.labels = None if self.labels is None else self.labels.pin_memory()
